...
namespace WebApplicationMyPage.Services
{
    ...
    {
        public List<ItemGeo> Values { get; set; }
    }
    ...
    { //
        ...
        public async Task<List<ItemGeo>> SearchByCity(string city)
        {
            var response_geo = await httpClient.GetAsync($"{BaseUrl}?q={city})&limit=5&appid={ApiKey}");
            var geo_json = await response_geo.Content.ReadAsStringAsync();
            var geo = JsonSerializer.Deserialize<List<ItemGeo>>(geo_json);
            return geo;
        }
    }
}
